...
strings = ['', 'valid_uaid', ' fooey barrey ', '!@#$%^&*()-+', '0', '1',
           '-66000', uuid[0],
           'True', 'False', 'true', 'false', '\"foo bar\"', str_gen(64000)]
data_types = ['messageType', 'HeLLO', '!@#$%^&*()-+', '0', '1', '-66000', '',
              1, 0, -1, True, False, None, ' fooey barrey ', str_gen(64000),
...
class TestPushAPI(unittest.TestCase):
    ...
    def test_empty_args(self):
        ...
        ret = self._msg({"messageType": "hello",
                         "channelIDs": [],
                         "uaid": "CASE_UAID"})
        self._compare_dict(ret, {'status': 200,
                                 'messageType': 'hello'})
        ...
        #register
        self._msg({"messageType": "hello",
                   "channelIDs": [add_epoch('empty_chan')],
                   "uaid": "CASE_UAID"})

        ret = self._msg({"messageType": "register",
                         "channelID": add_epoch("CASE_CHAN"),
                         "uaid": ""})
    ...
    def test_reg_noshake(self):
        ...
        # valid
        ret = self._msg({"messageType": "hello",
                         "channelIDs": [add_epoch("reg_noshake_chan_1")],
    ...
    def test_reg_plural(self):
        ...
        # XXX bug uaid can get overloaded with channels, adding epoch to
        # unique-ify it.
        self._msg({"messageType": "hello",
                  "channelIDs": [add_epoch("reg_plural_chan")],
                  "uaid": add_epoch("reg_plural")})
        ret = self._msg({"messageType": "register",
                        "channelID": add_epoch("reg_plural_chan"),
                        "uaid": add_epoch("reg_plural")})
        ...
        for dt in data_types:
            ...
            if 'error' in ret:
                # lots of errors here, lots of gross logic to validate them
                # here
                continue
            self._compare_dict(ret, {"messageType": "register", "status": 200})
    ...
    def test_unreg(self):
        ...
        # setup
        self._msg({"messageType": "hello",
                   "channelIDs": [add_epoch("unreg_chan")],
                   "uaid": "unreg_uaid"})
    ...
    def test_ping(self):
        ...
        self._msg({"messageType": "hello",
                  "channelIDs": [add_epoch("ping_chan_1")],
                  "uaid": "ping_uaid"})
        ret = self._msg({"messageType": "register",
                         "channelID": add_epoch("ping_chan_1")})
        self._compare_dict(ret, {"status": 200, "messageType": "register"})
        # send and ack too
    ...
    def test_ack(self):
        ...
        # no hello
        ret = self._msg({"messageType": "ack",
                        "updates": [{"channelID": add_epoch("ack_chan_1"),
                                     "version": 23}]})
        self._compare_dict(ret, {"error": "Invalid command",
                                 "status": 401,
                                 "messageType": "ack"})
        self.assertEqual(ret["updates"][0]["channelID"],
                         add_epoch("ack_chan_1"))
        self.assertEqual(ret["updates"][0]["version"], 23)

        # happy path
        self._msg({"messageType": "hello",
                  "channelIDs": [add_epoch("ack_chan_1")],
                  "uaid": "ack_uaid"})
        reg = self._msg({"messageType": "register",
                         "channelID": add_epoch("ack_chan_1")})
        ...
        ret = self._msg({"messageType": "ack",
                        "updates": [{"channelID": add_epoch("ack_chan_1"),
                                     "version": 23}]})
        self._compare_dict(ret, {"messageType": "notification",
                                 "expired": None})
        self.assertEqual(ret["updates"][0]["channelID"],
                         add_epoch("ack_chan_1"))
